...
let colors = ["lightpink", "blue", "green", "black"];  //to maintain order
...
for (let i = 0; i < toolBoxColors.length; i++) {
  toolBoxColors[i].addEventListener("click", function () {
    ...
    for (let i = 0; i < ticketArr.length; i++) {
      if (ticketArr[i].color == currentColor) {    //jis color ka h wo store kiya
        filteredArr.push(ticketArr[i]); 
      }
    }
    // console.log(filteredArr);
    let allTickets = document.querySelectorAll(".ticket-cont");
    for (let j = 0; j < allTickets.length; j++) {     //purana remove
      allTickets[j].remove();
    }
    for (let i = 0; i < filteredArr.length; i++) {    //new create
  ...
  toolBoxColors[i].addEventListener("dblclick", function () {    //all visible
    let allTickets = document.querySelectorAll(".ticket-cont");
    for (let j = 0; j < allTickets.length; j++) {
      allTickets[j].remove();                       //purana remove
    }
    for (let i = 0; i < ticketArr.length; i++) {
      ...
      let task = ticket.task;
      let id = ticket.id;
      createTicket(color, task, id);   // new create
    }
  });
}
...
for (let i = 0; i < allPriorityColors.length; i++) {   //4maai sae 1 ko active rakne ke liye 4 pe event listener laga do jis pe click uska class active kar do
  let priorityDivOneColor = allPriorityColors[i];
  priorityDivOneColor.addEventListener("click", function (e) {
    for (let j = 0; j < allPriorityColors.length; j++) { //loop to remove prev active
      allPriorityColors[j].classList.remove("active");
    }
    priorityDivOneColor.classList.add("active");
    modalPriorityColor = priorityDivOneColor.classList[0]; //at 0 idx we have color so..
  });
}
...
modalCont.addEventListener("keydown", function (e) {
  let key = e.key;
  if (key == "Enter") {
    createTicket(modalPriorityColor, taskAreaCont.value);   //1st simple add on UI
    taskAreaCont.value = "";
    modalCont.style.display = "none";
    addModal = !addModal;
  }
...
function createTicket(ticketColor, task, ticketId) {
  ...
  ticketColorBand.addEventListener("click", function () {
    //update UI
    let currentTicketColor = ticketColorBand.classList[1]; // 1 idx par actual color
  ...
  if (ticketId == undefined) {   // jb ui sae aa raha h tb push
